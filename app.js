...
const totalFooter = () => {
    ...
    const nProductos = carrito.reduce((a, b) => ({
        cantidad: a.cantidad + b.cantidad,
    }));
    // console.log(nProductos.cantidad)

    const nPrecio = carrito.reduce((a, b) => ({
    ...
    template.querySelector(".font-weight-bold span").textContent =
        nPrecio.precioTotal;
        ...
        btn.addEventListener("click", () => {
            ...
            const arrayFiltrado = carrito.map((item) => {
                if (item.id === parseInt(btn.dataset.id)) {
                    item.precioTotal =
                        item.precioTotal + item.precioTotal / item.cantidad;
                    item.cantidad++;
